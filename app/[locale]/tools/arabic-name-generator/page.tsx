...
const TOOL_SLUG = "arabic-name-generator"
const TOOL_NAME = "Arabic Name Generator"
...
export async function generateMetadata({
  ...
  const t = await getTranslations({ locale, namespace: "tools.arabicNameGenerator.meta" })
...
export default async function ArabicNameGeneratorPage({
  params,
}: {
  params: Promise<{ locale: string }>
}) {
  const { locale } = await params
  const t = await getTranslations({ locale, namespace: "tools.arabicNameGenerator" })
  ...
  const layoutItems = t.raw("layoutIdeas.items") as Array<{ title: string; description: string }>
  const stylingTips = t.raw("stylingTips.items") as string[]
  ...
  return (
    <>
      ...
      <main className="min-h-screen bg-gradient-to-b from-amber-50 to-white">
        ...
        <section className="container mx-auto px-4 pb-12">
          <div className="max-w-5xl mx-auto">
            <h2 className="text-2xl font-bold text-amber-800 mb-4">{t("layoutIdeas.title")}</h2>
            <div className="grid gap-4 md:grid-cols-3">
        ...
        <section className="container mx-auto px-4 pb-12">
          <div className="max-w-3xl mx-auto bg-white border border-amber-100 rounded-xl p-6 md:p-8">
            <h2 className="text-2xl font-bold text-amber-800 mb-4">{t("stylingTips.title")}</h2>
            <ul className="space-y-3 text-gray-600">
              {stylingTips.map((item) => (
